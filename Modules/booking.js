...
exports.getallbookings = function(userid, res) {
	...
	var getBooking = "SELECT CONCAT(s.addressline1, \" \", s.addressline2) AS address,s.spotid AS spotid, r.status, u.monthlypending, u.pendingtransfer, u.lifetimeearnings, u.lifetimetransactions, max(i.imageurl) AS imageurl FROM spot s JOIN image i ON i.spotid = s.spotid JOIN reservation r ON r.userid = s.userid JOIN user u ON u.userid = r.userid WHERE r.userid = "+ userid + " GROUP BY 1, 2, 3, 4, 5, 6, 7";
	console.log("Query is:" + getBooking);
